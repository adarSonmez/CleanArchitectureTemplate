...
public class Basket : BaseEntity
{
    ...
    {
        if (BasketItems == null || BasketItems.Count == 0 || BasketItems.Any(item => item.TotalPrice == null))
        {
            return null;
        }

        // Ensure all items have the same currency
        var firstCurrency = BasketItems.First().TotalPrice?.Currency ?? Currency.Usd;
        ...
        var total = BasketItems.Sum(item => item.TotalPrice?.Amount ?? 0);
